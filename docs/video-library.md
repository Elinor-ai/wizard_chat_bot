...
### Code-Only Settings (`llm-config.js`)

Provider, model, and behavior settings are configured in code, not via environment variables:

```javascript
// VIDEO_RENDER_CONFIG - provider & model selection
...
// VIDEO_BEHAVIOR_CONFIG - feature flags & paths
...
To change these settings, edit `services/api-gateway/src/config/llm-config.js` directly.
...
| `OPENAI_API_KEY`                | _(required for Sora)_                | API key for OpenAI Sora video generation.                                                       |
| `RENDER_POLL_TIMEOUT_MS`        | `600000`                             | Max wait (ms) while polling operations before timing out.                                       |
| `RENDER_POLL_INTERVAL_MS`       | `2000`                               | Poll delay for video operations.                                                                |

> **Deprecated:** The following env vars are no longer used. Remove them from your `.env` file:
> - `VIDEO_DEFAULT_PROVIDER`, `VIDEO_MODEL` (now in `VIDEO_RENDER_CONFIG`)
...
3. **Render** – `createVideoLibraryService` invokes the renderer. With Veo enabled, we build a "director" prompt, generate an 8-second base clip on Veo 3 (or Veo 3 Fast), then issue Veo 3.1 "Extend" hops until the channel's duration window is reached. Assets are downloaded into the output directory (configured in `VIDEO_BEHAVIOR_CONFIG.outputDir`), tagged with SynthID provenance, and saved alongside cost estimates (seconds × tier rate). When the Gemini API is unavailable we fall back to the storyboard bundle.
...
| -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **LLM failures**           | Set `VIDEO_BEHAVIOR_CONFIG.llmEnabled = false` in `llm-config.js` to force deterministic manifests. Logs include `video_storyboard` payload + preview; audit log lists fallback usage.                                                   |
...
- **LLM off switch** – Integration test toggling `VIDEO_BEHAVIOR_CONFIG.llmEnabled = false` to confirm deterministic storyboard/caption created and audits record fallback.
