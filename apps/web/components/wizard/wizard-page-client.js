"use client";

import { WizardShell } from "./wizard-shell";

export function WizardPageClient({ jobId = null, companyId = null, mode = "create" }) {
  const isImportMode = mode === "import";
  const heading = jobId
    ? isImportMode
      ? "Review imported job"
      : "Continue job wizard"
    : isImportMode
      ? "Review imported job"
      : "Job creation wizard";
  const description = jobId
    ? isImportMode
      ? "We pre-filled this draft from an existing posting. Review the details, make any tweaks, and continue directly to distribution."
      : "Pick up where you left off. Your latest saved answers and any recent draft edits are restored automatically."
    : isImportMode
      ? "We imported a job description for you. Confirm the essentials and jump straight to recommendations."
      : "Capture must-have details first. Sidecar LLM agents suggest enriched context without affecting the confirmed record until you approve.";

  return (
    <div className="space-y-6">
      <header className="flex flex-col gap-2">
        <h1 className="text-3xl font-semibold text-neutral-900">{heading}</h1>
        <p className="text-sm text-neutral-600">{description}</p>
      </header>

      <WizardShell
        jobId={jobId ?? null}
        initialCompanyId={companyId ?? null}
        mode={mode}
      />
    </div>
  );
}
