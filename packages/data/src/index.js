...
function initializeFirebase({ projectId, serviceAccountPath, emulatorHost }, logger) {
...
export function createFirestoreAdapter(options = {}) {
  ...
  logger.info({ envProject: env.FIRESTORE_PROJECT_ID }, "Loaded Firestore env config");
  const rootDir = process.env.WIZARD_ROOT_DIR ?? process.cwd();
  const serviceAccountPathRaw = options.serviceAccountPath ?? env.GOOGLE_APPLICATION_CREDENTIALS;
  ...
  return {
    ...
    async queryDocuments(collection, field, operator, value) {
      const querySnap = await db.collection(collection).where(field, operator, value).get();
      return querySnap.docs.map((doc) => ({ id: doc.id, ...normalize(doc.data()) }));
    },
...
export function createRedisAdapter(options = {}) {
  ...
  if (!url) {
    logger.warn("Redis URL missing; adapter running in no-op mode");
  }

  return {
    async enqueue(queueName, payload) {
      logger.info({ queueName }, "Queue push (stub)");
      return { queueName, payload };
    },
    async dequeue(queueName) {
      logger.info({ queueName }, "Queue pull (stub)");
      return null;
    },
    async cache(key, value, ttlSeconds = 60) {
      logger.info({ key, ttlSeconds }, "Cache set (stub)");
      return { key, value };
    }
  };
}
