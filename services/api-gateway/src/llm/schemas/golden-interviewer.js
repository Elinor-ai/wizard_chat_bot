...
const ExtractionSchema = z.object({
  updates: z.record(z.any()).optional().describe(
    "Key-value pairs where keys are dot-notation paths in the Golden Schema"
  ),
  confidence: z.record(z.number()).optional().describe(
    "Confidence scores (0.0-1.0) for each extraction"
  ),
});
...
const UIToolSchema = z.object({
  ...
  props: z.record(z.any()).describe("Props for the UI tool, must match tool schema"),
});
...
export const GoldenInterviewerOutputSchema = z.object({
  message: z.string().describe(
    "Conversational response to the user (1-3 sentences)"
  ),
  ...
  next_priority_fields: z.array(z.string()).optional().describe(
    "Top 3 fields to fill next"
  ),
  completion_percentage: z.number().optional().describe(
    "Estimated schema completion (0-100)"
  ),
  interview_phase: z.string().optional().describe(
